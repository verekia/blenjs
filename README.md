...
```bash
bun run watch          # regenerates schema on registry change; mirrors game.json -> app/public
```

Then editing `game.json` (or saving from Blender) hot-reloads the running game.
...
| `bun run watch`          | dev loop glue: codegen on registry change + `game.json` → public sync |
